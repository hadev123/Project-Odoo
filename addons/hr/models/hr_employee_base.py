...
class HrEmployeeBase(models.AbstractModel):
    ...
    department_id = fields.Many2one(
        'hr.department', 'Department', domain="['|', ('company_id', '=', False), ('company_id', '=', company_id)]")
    ...
    job_id = fields.Many2one(
        'hr.job', 'Job Position', domain="['|', ('company_id', '=', False), ('company_id', '=', company_id)]")
    ...
    mobile_phone = fields.Char('Work Mobile', compute="_compute_work_contact_details",
                               store=True, inverse='_inverse_work_contact_details')
    work_email = fields.Char('Email', compute="_compute_work_contact_details",
                             store=True, inverse='_inverse_work_contact_details')
    work_contact_id = fields.Many2one(
        'res.partner', 'Work Contact', copy=False)
    ...
    resource_calendar_id = fields.Many2one(
        'resource.calendar', domain="['|', ('company_id', '=', False), ('company_id', '=', company_id)]")
    ...
    def _compute_part_of_department(self):
        ...
        if not active_department:
            ...
            child_departments = active_department + \
                get_all_children(active_department)
    ...
    def _compute_presence_state(self):
        ...
        """
        # Check on login
        check_login = literal_eval(self.env['ir.config_parameter'].sudo(
    ...
    def _compute_presence_icon(self):
        ...
        working_now_list = self.filtered(
            lambda e: e.hr_presence_state == 'present')._get_employee_working_now()
        for employee in self:
            show_icon = True
    ...
    def _get_employee_working_now(self):
        ...
        for tz in all_employee_tz:
            ...
            for calendar_id in resource_calendar_ids:
                ...
                from_datetime = utc.localize(
                    start_dt).astimezone(timezone(tz or 'UTC'))
                to_datetime = utc.localize(
                    stop_dt).astimezone(timezone(tz or 'UTC'))
